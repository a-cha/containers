...
#pragma region Coplien
	...
	template<typename value_type>
	ListNode <value_type> &
	ListNode<value_type>::operator=(const ListNode &other) {
		if (this != other) {
			_data = other._data;
			_next = other._next;
			_prev = other._prev;
...
#pragma region Constructors
	...
	template<typename value_type>
	ListNode<value_type>::ListNode(value_type const &val, ListNode *prev,
								   ListNode *next) :
			_data(val),
			_prev(prev),
			_next(next) {}
...
#pragma region Methods
	...
	void ListNode<value_type>::swapValues(ListNode *n) {
		...
		std::memmove(static_cast<void *>(&tmp), static_cast<void *>(&n->_data),
					 1);
		std::memmove(static_cast<void *>(&n->_data),
					 static_cast<void *>(&_data), 1);
